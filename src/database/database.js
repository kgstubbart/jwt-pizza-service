...
class DB {
	...
	async loginUser(userId, token) {
		...
		try {
			await this.query(
				connection,
				`INSERT INTO auth (token, userId, lastSeen) VALUES (?, ?, NOW()) ON DUPLICATE KEY UPDATE userId=VALUES(userId), lastSeen=NOW()`,
				[token, userId],
			);
		} finally {
			connection.end();
	...
	async initializeDatabase() {
        ...
        try {
          const ensure = async (table, columnDef) => {
            const [rows] = await connection.execute(
              `SELECT COUNT(*) AS c FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?`,
              [config.db.connection.database, table, 'lastSeen']
            );
            if (rows[0].c === 0) {
              await connection.query(`ALTER TABLE \`${table}\` ADD COLUMN lastSeen ${columnDef}`);
	...
	async touchAuthToken(token) {
		...
		try {
			await this.query(
				connection,
				`UPDATE auth SET lastSeen=NOW() WHERE token=?`,
				[token],
			);
		} finally {
			connection.end();
	...
	async touchUserLastSeen(userId) {
		...
		try {
			await this.query(
				connection,
				`UPDATE user SET lastSeen=NOW() WHERE id=?`,
				[userId],
			);
		} finally {
			connection.end();
